...
function cCircle(v: Array<Array<number>>, i: number, j: number, k: number): any {
	var x1: number = v[i][0],
		y1: number = v[i][1],
		x2: number = v[j][0],
		y2: number = v[j][1],
...
function sTriangle(v: Array<Array<number>>): Array<Array<number>> {
...
function deDup(e: Array<any>): void {
...
export default class Delaunay {
	...
	constructor(vertices: Array<Array<number>>) {
		...
		st = sTriangle(vertices);
		vertices.push(st[0], st[1], st[2]);

		open = [cCircle(vertices, n + 0, n + 1, n + 2)];
		closed = [];
		edges = [];
		...
		for (i = indices.length; i--; edges.length = 0) {
			...
			for (j = edges.length; j;) {
				b = edges[--j];
				a = edges[--j];
				open.push(cCircle(vertices, a, b, c));
			}
		}
